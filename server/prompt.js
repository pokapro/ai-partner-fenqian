...
> 📄 **需要准备的协议文件**：列出用户要求的所有协议类型。如果用户明确说了需要《股东协议书》《一致行动人协议》《代持协议》，则必须包含这三份，不可遗漏。如有必要可增加其他建议文件，但不允许减少用户明确要求的协议。
...
## 股权比例（用户已确认）
${partners.filter(p => p.equity).length > 0 ? partners.map(p => `- ${p.name || '合伙人'}：${p.equity}%`).join('\n') : '未填写，请根据出资额+出力情况合理建议'}

${nomineeDetail ? `## 代持关系
${nomineeDetail}` : ''}

${needsAgreements ? `## 用户需要的协议文件
${needsAgreements}

⚠️ 重要规则：用户明确要求以上协议类型，报告中关于协议文件的部分必须包含这些协议，不可遗漏。可额外建议其他文件，但不允许减少。` : ''}
