// 框架树待补全清单 — 记录用户提到但当前框架未覆盖的合伙相关要素
// 作用：当 AI/用户触发 "相关但没有" 时，把要素记录到这里，定期 review 入决策树
...
const COVERED_KEYWORDS = {
  ...
  // 注：以下词虽然已"知道"，但 prompt_dt.js 没有专门条款模块，应记录为 gap：
  //   - "期权池" / "员工持股" / "ESOP" — 没有独立模块
  //   - "反稀释" / "对赌" / "优先清算" — 没有独立模块
  //   - "AB 股" / "同股不同权" — 没有独立模块
  // 所以这些词也归到 CATEGORY_HINTS → 走 gap 流程
...
const CATEGORY_HINTS = [
  ...
  { kw: ['估值方法', '净资产法', '市场法', 'DCF', '评估机构'], cat: 'exit_detail' },
